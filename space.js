...
connection.query("DELETE FROM programa WHERE canal_idcanal = 11");
...
var url = "http://www.gatotv.com/canal/space"
...
download(url, function(data){
  if (data){
    ...
    $("tr.tbl_EPG_row").each(function(i,e){
      ...
      var sport = $(e).find("td.deporte > div").text();
      ...
      if (program.trim() == ""){
        ...
        var programa = {idprograma: id, nombre: program.trim(), horaDeInicio: time, canal_idcanal: 11};
      ...
      if (movie.trim() == ""){
        ...
        var programa = {idprograma: id, nombre: movie.trim(), horaDeInicio: time, canal_idcanal: 11};
        connection.query("INSERT INTO programa SET ?", programa);
        connection.commit();
      }
      ...
      if (sport.trim() == ""){
        ...
        //guardar en la BD
        if(sport.length > 45){
          sport = sport.slice(0,26);
        }
        var programa = {idprograma: id, nombre: sport.trim(), horaDeInicio: time, canal_idcanal: 11};
        connection.query("INSERT INTO programa SET ?", programa);
        connection.commit();
      }
